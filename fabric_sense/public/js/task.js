...
frappe.ui.form.on("Task", {
	refresh: function (frm) {
		...
		const possible_field_names = ["tailoring_sheet", "custom_tailoring_sheet", "tailoring_sheet_link"];
	...
	after_save: function (frm) {
		...
		if (frm.doc.status === "Working") {
			if (task_status_before_save === "Open" || task_status_before_save === null || frm.doc.__islocal) {
				// Status changed from Open to Working, or new document with Working status
				handle_status_working(frm);
			}
		}
...
function get_contractor_query(frm) {
	...
	if (frm.doc.custom_service) {
		// Service is selected
		if (cached_contractors && Array.isArray(cached_contractors) && cached_contractors.length > 0) {
		...
		} else if (cached_contractors && Array.isArray(cached_contractors) && cached_contractors.length === 0) {
			...
			callback: function (r) {
				if (r.message) {
					...
					if (frm.fields_dict.custom_total_contractor_amount) {
						frm.set_value("custom_total_contractor_amount", r.message.total_contractor_amount);
					}
...
function handle_status_working(frm) {
	...
	if (!tailoring_sheet_value) {
		...
		frappe.msgprint({
			...
			indicator: "orange",
			message: __("Please select a Tailoring Sheet before changing status to Working. Stock Entry form cannot be opened without a Tailoring Sheet.") + 
				"<br><br>" + __("Debug Info:") +
				"<br>" + __("custom_tailoring_sheet field: {0}", [frm.doc.custom_tailoring_sheet || "empty"]) +
				"<br>" + __("tailoring_sheet field: {0}", [frm.doc.tailoring_sheet || "empty"])
		});
		return;
	}
	...
	frappe.call({
			...
			if (r.message) {
				if (r.message.error) {
					...
					if (r.message.docstatus === 1) {
						// Material Request is submitted, open Stock Entry form
						open_stock_entry_form(r.message, tailoring_sheet_value);
					} else {
						// Material Request exists but is not submitted
						frappe.msgprint({
							title: __("Material Request Not Submitted"),
							indicator: "orange",
							message: __("Material Request {0} exists but is not submitted. Please submit it before creating Stock Entry.", [r.message.material_request_name])
						});
					}
				} else {
					// Show toast message if no Material Request found
					frappe.show_alert({
						message: __("No material request associated with the tailoring sheet"),
						indicator: "orange"
					}, 5);
				}
			} else {
		...
		error: function (r) {
			...
			if (typeof error_msg === "object" && error_msg.exc_type) {
				error_msg = error_msg.exc_type + ": " + (error_msg.exc || error_msg.message || "Unknown error");
			}
...
function open_stock_entry_form(material_request_data, tailoring_sheet) {
	// Create Stock Entry from Material Request
	if (material_request_data.material_request_name && material_request_data.items && material_request_data.items.length > 0) {
		...
		frappe.model.with_doctype("Stock Entry", function () {
			...
			material_request_data.items.forEach(function (item, index) {
				let stock_entry_item = frappe.model.add_child(
					stock_entry,
					"items"
				);
			...
			stock_entry_item.transfer_qty = item.qty * (stock_entry_item.conversion_factor || 1);
				stock_entry_item.item_name = item.item_name;
				stock_entry_item.description = item.description;
				stock_entry_item.material_request = material_request_data.material_request_name;
				stock_entry_item.material_request_item = item.name; // Item row name from Material Request
			});
			...
			stock_entry.items.forEach(function (item) {
				...
				frappe.model.trigger("item_code", stock_entry.doctype, stock_entry.name, item.name);
