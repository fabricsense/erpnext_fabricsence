...
def get_data(data):
    ...
    # Add or update the fieldnames for Measurement Sheet and Tailoring Sheet
    data["non_standard_fieldnames"].update({
        "Measurement Sheet": "project",
        "Tailoring Sheet": "project",
    })
    ...
    data["internal_links"].update({
        # "Measurement Sheet": ["project", "project"],
        "Tailoring Sheet": ["project", "project"],
    })
    ...
    fabric_sense_section = next((t for t in data["transactions"] 
                              if t.get("label") == _("Fabric Sense")), None)
    
    if fabric_sense_section:
        # Update existing Fabric Sense section
        existing_items = set(fabric_sense_section.get("items", []))
        fabric_sense_section["items"] = list(existing_items.union({"Measurement Sheet", "Tailoring Sheet"}))
    else:
        # Add new Fabric Sense section if it doesn't exist
        data["transactions"].append({
            "label": _("Fabric Sense"),
            "items": ["Measurement Sheet", "Tailoring Sheet"],
        })
